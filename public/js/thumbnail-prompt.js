...
export function buildThumbnailPrompt({
    ...
    const roleLines = validRoles.map((role, index) =>
        `- image${index + 1}（${role}）：提示詞中提及此人時必須保留這個完整指代，不可改名或翻譯。`
    );
    const logoIndex = validRoles.length + 1;
    const roleInstruction = validRoles.length
        ? `使用者會依序上傳以下角色參考圖片：
${roleLines.join('\n')}
最終繪圖提示詞開頭必須完整寫上：「請參考我上傳的角色圖片，並啟動嚴格臉部一致性模式，確保人物的五官、髮型與特徵完全保持不變。」
...
【四段式視覺架構】
1. 主體與動作：清楚描述畫面主體、表情、動作及情緒反差，讓縮小後仍能瞬間理解故事。
2. 地點／背景：設計能強化影片主題的具體場景，避免無關裝飾與資訊過載。
3. 構圖／鏡頭：${shotDescription}
4. 藝術風格：${styleDescription}
...
- ${titleInstruction}
- 依主體位置在另一側或畫面上方預留乾淨負空間，將實際封面文字用兩組半形雙引號標示，例如：""爆款密碼""。
- 明確描述字體、大小、顏色與效果；使用巨大的現代無襯線粗體、高對比配色、深色粗描邊或醒目色塊，確保手機縮圖尺寸仍清楚可讀。
