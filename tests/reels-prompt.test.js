...
import { buildReelsPrompt, extractReelsPromptBlocks, normalizeReelsPromptOutput } from '../public/js/reels-prompt.js';
...
test('Reels prompt outputs only independent image prompts with a shot-count prefix', () => {
    ...
    assert.match(prompt, /教師研習摘要 Reels/);
...
test('Reels output blocks can be copied one image at a time', () => {
    const blocks = extractReelsPromptBlocks('分鏡規劃\n\n[第 1 張]\n封面\n\n[第 2 張]\n問題');
...
test('Reels page is wired after the YouTube thumbnail page', () => {
    ...
    assert.match(html, /id="reels-brand-colors"/);
